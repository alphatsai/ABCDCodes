...
using namespace std;
void countAndStore(string name, fstream& f, TH1F* h){
	f<<"***************************************"<<endl;
	f<<"************* "<<name<<" **************"<<endl;
	f<<"***************************************"<<endl;
	...
	float dCBbyA = sqrt(sigC*sigC+sigB*sigB+sigA*sigA)*C*(B/A);	
...
void abcdCount(){
	...
	string loadPath="/afs/cern.ch/work/j/jtsai/myAna/bpTobH/mywk/CMSSW_5_3_11/src/TaBV/result/root/abcd";
	string savePath="/afs/cern.ch/work/j/jtsai/myAna/bpTobH/mywk/CMSSW_5_3_11/src/TaBV/result/txt/abcd";
...
/*	for( int i=0; i<abcdFiles_Sum.size(); i++ ){
	...
	for( int i=0; i<h.size(); i++){
		...
		for( int j=0; j<abcdFiles_Bg.size(); j++ ){
			countAndStore("Background", h_txt[i], hBg[i][j]);
		}
		for( int k=0; k<abcdFiles_Sig.size(); k++){
			countAndStore(abcdFiles_Sig[k], h_txt[i],hSig[i][k]);
		}
/*		for( int k=0; k<abcdFiles_Sum.size(); k++){
			countAndStore(abcdFiles_Sum[k], h_txt[i],hSum[i][k]);
		}*/
		h_txt[i].close();
	}
}
