...
#define MAX_BX		128
#define N_TRIGGER_BOOKINGS	5834 // from 226 (CMSSW_3_6_1 : 8E29 & 1E31) to 644 (CMSSW_3_8_3 : 2E32) ; 904 (CMSSW_4_1_3 : 5E32); 1212 (CMSSW_4_1_4 : 1E33); 2014 (CMSSW_4_2_3)	; 2390 (CMSSW_4_2_3) ; 2722 (prompt_v5); 2996 (prompt_v6)
...
class EvtInfoBranches {
	public:
		...
		float McDauPhi[14];	 // MC daughters: 0-1: hard jet from b'bar/t'bar, 2-9: W daughters, 10-13: Z daughters float McDauPhi[14];	 
		float McDauEta[14];	 // MC daughters: 0-1: hard jet from b'bar/t'bar, 2-9: W daughters, 10-13: Z daughters float McDauPhi[14];	 
...
class JetInfoBranches {
	public:
		...
		//// Function For Selection ///// by Alpha
		bool isGoodAK5Ht();
		bool is2bCA8(int i);
		bool isGoodCA8(int i);
		bool isGood2bCA8(int i);
		bool isbAK5(int i);
		bool isGoodAK5(int i);
		bool isGoodbAK5(int i);
...
class PhotonInfoBranches {
	public:
		...
		void RegisterTree(TTree *root, std::string name="PhotonInfo") {
			...
			root->Branch((name+"EcalIso").c_str()    , &EcalIso[0]		,(name+"EcalIso["+name+".Size]/O" ).c_str()      );
			root->Branch((name+"HcalIso").c_str()   	, &HcalIso[0]		,(name+"HcalIso["+name+".Size]/O"  ).c_str()     );
			root->Branch((name+"TrackIso").c_str()   , &TrackIso[0]	,(name+"TrackIso["+name+".Size]/O"  ).c_str()     );
			root->Branch((name+"GenPt").c_str()		  , &GenPt[0]		  ,(name+"GenPt["+name+".Size]/F"  ).c_str()     );
			root->Branch((name+"GenEta").c_str()    	, &GenEta[0]		,(name+"GenEta["+name+".Size]/F"  ).c_str()     );
			root->Branch((name+"GenPhi").c_str()    	, &GenPhi[0]		,(name+"GenPhi["+name+".Size]/F"  ).c_str()     );
			root->Branch((name+"GenPdgID").c_str()   , &GenPdgID[0]  ,(name+"GenPdgID["+name+".Size]/I" ).c_str()      );
