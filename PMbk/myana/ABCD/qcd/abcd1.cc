...
void abcd1(){
	for( int iSample=0; iSample<(Category[2].SampleSize)/2; iSample++){   //FullPart
		...
		string LoadPath	 = "/raid1/w/jtsai/bpTobH/skimGeneral/CA8_Pt150_1/"+QCDMC[iSample].name+".root";
		string SavePath  = "/afs/cern.ch/work/j/jtsai/myAna/bpTobH/mywk/CMSSW_5_3_11/src/PMbk/result/root/abcd/abcd_"+QCDMC[iSample].name+".root";
		...
		cout<<"Evt weight: "<<QCDMC[iSample].xsec*lumi*1000/totalEvt<<endl;
		...
		for( int entry=0; entry<tree->GetEntries(); entry++ ){
			...
			if( buildTree ){
				if( A+B+C+D > 0 ){ 
					if( McFlag ) xsec_ = QCDMC[iSample].xsec;
					else xsec_ = 1.;
					GenEvt_ = totalEvt;
					McFlagana = McFlag;
					PUana = PU;
