...
void abcd(){
	for( int iSample=0; iSample<Category[1].SampleSize; iSample++){   //FullPart
		...
		string LoadPath	 = "/raid1/w/jtsai/bpTobH/skimGeneral/CA8_Pt150_1/"+SigMC[iSample].name+".root";
		string SavePath  = "/afs/cern.ch/work/j/jtsai/myAna/bpTobH/mywk/CMSSW_5_3_11/src/PMbk/result/root/abcd/abcd_"+SigMC[iSample].name+".root";
		...
		cout<<"Evt weight: "<<SigMC[iSample].xsec*lumi*1000/totalEvt<<endl;
		...
		for( int entry=0; entry<tree->GetEntries(); entry++ ){
			...
			if( buildTree ){
				if( A+B+C+D > 0 ){ 
					if( McFlag ) xsec_ = SigMC[iSample].xsec;
					else xsec_ = 1.;
					GenEvt_ = totalEvt;
					McFlagana = McFlag;
					PUana = PU;
