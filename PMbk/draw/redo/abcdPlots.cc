...
//	abcdFiles.push_back("abcd_SumBg_BH_1500");
	...
	TLine* ly2[abcdFiles.size()][th2_size_];
		...
		for( int j=0; j<th2_size_; j++){
			...
			if( TH2Info[j].xunit.size()!=0 ){
				if (binwidthX<1){
					...
					sprintf( xtitle, "%s / [%1.0f%s]", TH2Info[j].xtitle.c_str(),binwidthX,TH2Info[j].xunit.c_str());
				}
			}else{
				if (binwidthX<1){
					sprintf( xtitle, "%s / [%0.2f]", TH2Info[j].xtitle.c_str(),binwidthX);
				}else{
					sprintf( xtitle, "%s / [%1.0f]", TH2Info[j].xtitle.c_str(),binwidthX);
				}
			}
			if( TH2Info[j].yunit.size()!=0 ){
				if (binwidthY<1){
					...
					sprintf( ytitle, "%s / [%1.0f%s]", TH2Info[j].ytitle.c_str(),binwidthY,TH2Info[j].yunit.c_str());
				}
			}else{
				if (binwidthY<1){
					sprintf( ytitle, "%s / [%0.2f]", TH2Info[j].ytitle.c_str(),binwidthY);
				}else{
					sprintf( ytitle, "%s / [%1.0f]", TH2Info[j].ytitle.c_str(),binwidthY);
				}
			}
			...
			yMax = h[i][j]->GetYaxis()->GetBinUpEdge(h[i][j]->GetYaxis()->GetLast());
			yMin = h[i][j]->GetYaxis()->GetBinLowEdge(h[i][j]->GetYaxis()->GetFirst());
			...
			ly2[i][j] = new TLine(xMin,Other2Info[j].yCut2,xMax,Other2Info[j].yCut2);
			lx[i][j]->SetLineStyle(8);	
			lx[i][j]->SetLineWidth(5);	
			ly[i][j]->SetLineStyle(8);	
			ly[i][j]->SetLineWidth(5);
			ly2[i][j]->SetLineStyle(8);	
			ly2[i][j]->SetLineWidth(5);
