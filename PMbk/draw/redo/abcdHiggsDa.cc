...
//	string savePath="/afs/cern.ch/work/j/jtsai/myAna/bpTobH/mywk/CMSSW_5_3_11/src/PMbk/result/png/abcd/SBCut70";
//	string loadPath="/afs/cern.ch/work/j/jtsai/myAna/bpTobH/mywk/CMSSW_5_3_11/src/PMbk/result/root/abcd/HTNAME";
...
//	h.push_back("ABCD1.HT1.HiggsDa");
	h.push_back("ABCD1.HT1.HiggsDa.A");
	h.push_back("ABCD1.HT1.HiggsDa.B");
	h.push_back("ABCD1.HT1.HiggsDa.C");
	h.push_back("ABCD1.HT1.HiggsDa.D");
//	h.push_back("ABCD1.HT2.HiggsDa");
	h.push_back("ABCD1.HT2.HiggsDa.A");
	h.push_back("ABCD1.HT2.HiggsDa.B");
	h.push_back("ABCD1.HT2.HiggsDa.C");
	h.push_back("ABCD1.HT2.HiggsDa.D");
//	h.push_back("ABCD1.HT3.HiggsDa");
	h.push_back("ABCD1.HT3.HiggsDa.A");
	h.push_back("ABCD1.HT3.HiggsDa.B");
	h.push_back("ABCD1.HT3.HiggsDa.C");
	h.push_back("ABCD1.HT3.HiggsDa.D");
...
//	for( int i=4; i<5; i++){
		...
		for( int j=0; j<abcdFiles_Data.size(); j++ ){
			...
			setXYTitle(hData[i][j], "Higgs daughter PdgID", "Yields", "GeV/c^{2}");
			hData[i][j]->Draw("HIST");
			string save = savePath+"/"+h[i]+"_"+abcdFiles_Data[j]+".png";
			c1->SaveAs(save.c_str());
	
			setXYTitle(hData[i][j], "Higgs daughter PdgID", "PDF", "GeV/c^{2}");
		...
		for( int j=0; j<abcdFiles_Bg.size(); j++ ){
			...
			setXYTitle(hBg[i][j], "Higgs daughter PdgID", "Yields", "GeV/c^{2}");
			hBg[i][j]->Draw("HIST");
			string save = savePath+"/"+h[i]+"_"+abcdFiles_Bg[j]+".png";
			c1->SaveAs(save.c_str());	

			setXYTitle(hBg[i][j], "Higgs daughter PdgID", "PDF", "GeV/c^{2}");
		...
		for( int k=0; k<abcdFiles_Sig.size(); k++){
			...
			setXYTitle(hSig[i][k], "Higgs daughter PdgID", "Yields", "GeV/c^{2}");
			hSig[i][k]->Draw("HIST");
			string save = savePath+"/"+h[i]+"_"+abcdFiles_Sig[k]+".png";
			c1->SaveAs(save.c_str());	

			setXYTitle(hSig[i][k], "Higgs daughter PdgID", "PDF", "GeV/c^{2}");
