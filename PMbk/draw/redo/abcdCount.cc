...
using namespace std;
void countAndStore(string name, fstream& f, TH1F* h, TH1F* bg, bool sig=0){
	f<<"***************************************"<<endl;
	f<<"************* "<<name<<" **************"<<endl;
	f<<"***************************************"<<endl;
...
void abcdCount(){
	...
	string savePath="/afs/cern.ch/work/j/jtsai/myAna/bpTobH/mywk/CMSSW_5_3_11/src/PMbk/result/txt/abcd";
	...
	h.push_back("ABCDana_CutRegion");
	h.push_back("ABCDval_CutRegion");
	...
	for( int i=0; i<h.size(); i++){
		...
		for( int f=0; f<abcdFiles_Data.size(); f++){
			countAndStore("Data", h_txt[i],hData[i][f], hData[i][f]);
		}
		for( int j=0; j<abcdFiles_Bg.size(); j++ ){
			countAndStore("Background", h_txt[i], hBg[i][j], hBg[i][j]);
			for( int k=0; k<abcdFiles_Sig.size(); k++){
				countAndStore(abcdFiles_Sig[k], h_txt[i],hSig[i][k], hBg[i][j], 1);
			}
		}
